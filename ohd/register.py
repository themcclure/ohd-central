...
# TODO: Deprecated
def load_officials(id_list, cred_file=None, delay=None):
    """
    Takes a list of OHD doc IDs and iterates through it and loads each Official. Returns the list of successfully loaded
    Officials.
    :param id_list: list of OHD doc IDs to load
    :param cred_file: The location of the JSON file with the service account credentials in it
    :param delay: Number of seconds to pause between each official; this exists only to keep within the Google API quota
    :return: A list of loaded officials objects
    """
    # process default arguments
    if cred_file is None:
        cred_file = config.cred_file
    if delay is None:
        delay = config.google_api_delay
                    ...
                    if hasattr(e.response, 'status_code'):
                        if e.response.status_code == 429:
                            ...
                            if doc == last_paused_on:
                                time.sleep(delay*3)  # delay longer if this is repeated error on the same doc
                            else:
                                last_paused_on = doc
                                time.sleep(delay)
                        elif e.response.status_code == 503:
                            # resource unavailable, try again after a big pause
                            # NOTE: I don't know what causes this error... could just be internet connection issues?
                            err_fatal = False
                            err_unavailable += 1
...
# TODO: DEPRECATED
def old_ohd_conn_generator(id_list, cred_file='./service-account.json', delay=6):
    """
    This ia a Generator that takes a list of OHD doc IDs and iterates through it and yields an active connection to
    ...
    :param delay: Number of seconds to pause between each official; this exists only to keep within the Google API quota
    :return: A list of tuples of (doc_id, result of update)
    """
